...
import { fetchWeatherDataForLocation, fetchWeatherDataByCoords, fetchRefreshedTodayData } from './services/weatherService';
...
const App: React.FC = () => {
  ...
  const runWeatherSearch = useCallback(async (searchFn: () => Promise<{data: WeatherData, coords: {lat: number, lon: number}}>, name: string) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
        ...
        setError(`Sorry, I couldn't get the weather for ${name}. ${errorMessage}`);
    } finally {
        setIsLoading(false);
    }
  }, []);

  const handleFetchWeather = useCallback((newLocation: string) => {
    if (!newLocation) {
      setError('Please enter a location.');
      return;
    }
    runWeatherSearch(() => fetchWeatherDataForLocation(newLocation, new Date()), newLocation);
  }, [runWeatherSearch]);
  ...
  const handleGeolocate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            runWeatherSearch(() => fetchWeatherDataByCoords(position.coords.latitude, position.coords.longitude, new Date()), 'Your Location');
        },
        (err) => {
            console.warn(`Geolocation error: ${err.message}`);
            setError(`Could not get your location. ${err.message}. Please search for a location manually.`);
            setIsLoading(false);
        }
    );
  }, [runWeatherSearch]);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
        const newTodayData = await fetchRefreshedTodayData(coordinates.lat, coordinates.lon, new Date());
  ...
  useEffect(() => {
    ...
    if (lastLocation && !location) { // Only run on initial load
                ...
                >
                    {isRefreshing ? (
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-white">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0011.664 0M2.985 19.644a8.25 8.25 0 0111.664 0m0 0l3.181-3.183m-3.181 3.183V19.644" />
                        </svg>
                    )}
                </button>
              )}
            </div>
          <p className="text-slate-400 mt-2 text-lg">Yesterday vs. Today, powered by AI</p>
        </header>

        <main>
          <div className="flex items-center justify-end gap-3 mb-4">
              <label htmlFor="auto-refresh-toggle" className="text-slate-400 text-sm cursor-pointer select-none">
          ...
          {yesterdayData && todayData && !isLoading && (
            <div className="mt-8 animate-fade-in flex flex-col gap-8">
              ...
              <div 
                className="grid grid-cols-1 md:grid-cols-2 gap-6 order-2 md:order-1"
              >
                <div
                ...
                >
                    <WeatherCard day="Yesterday" data={yesterdayData} isExpanded={isExpanded} />
                </div>
                 <div
                    onClick={toggleExpanded}
                    role="button"
                    ...
                    className="cursor-pointer"
                >
                    <WeatherCard day="Today" data={todayData} isExpanded={isExpanded} />
                </div>
              </div>
          ...
          {!yesterdayData && !todayData && !isLoading && !error && (
            <div className="text-center mt-12 p-8 bg-slate-800/50 rounded-lg border border-slate-700">
              <p className="text-slate-400 text-lg">Enter a location or use the location button to see how the weather compares.</p>
            </div>
          )}
        </main>
      </div>
